...
**Below commands are not necessary to run as I have pushed docker image to docker hub. we can skip this part and jump to next step - "Deployment of application in Kubernetes Cluster"**
    ...
    Goto dir "/Kubernetes_Assignment_Basics/app/"
    > cd "/Kubernetes_Assignment_Basics/app/"   ## replace path with complete path on your local
...
- Command to deploy:
    goto dir "/Kubernetes_Assignment_Basics/" ## replace path with complete path on your local
...
- Check logs on the pod:
    ...
    **pod name can be fetched from `kubectl get pods` command**
